...
void main() async {
  ...
  await Engage.instance.configureSDK(EngageEnvironment.debug, null);
...
class _EngageAppState extends State<EngageApp>
    implements EngageRegistrationHandler {
    ...
    return Center(
      child: Wrap(
        ...
        children: [
          ...
          ElevatedButton(
            onPressed:
                isContactPermissionGranted ? null : enableContactPermission,
            child: Wrap(
              children: [
          ...
          ElevatedButton(
            onPressed: isNotificationPermissionGranted
                ? null
  ...
  void onNumberEntered() {
    ...
    Engage.instance
        .register(phoneNumber: number, engageRegistrationHandler: this);
  }

  void onCodeEntered() {
    var code = _enterCodeController.text;
    Engage.instance
        .completeChallengeWithCode(code, (usernumber) => {print('sms verify')});
  }
  ...
  void enableContactPermission() async {
    ...
    if (!await Permission.contacts.isGranted) {
      Permission.contacts.onGrantedCallback(() {
        ...
        showSnakBarMessage(
            'Contact Permission is required for using engage service');
        setState(() {
  ...
  void enableNotificationPermission() async {
    ...
    if (!await Permission.notification.isGranted) {
      Permission.notification.onGrantedCallback(() {
        ...
        showSnakBarMessage(
            'Notification Permission is required for using engage service');
        setState(() {
  ...
  void openSetting() {
    openAppSettings().then((value) async {
      if (await Permission.contacts.isDenied ||
        ...
        showSnakBarMessage(
            'Both Permissions are required for using engage service');
      }
      checkPermission();
    });
  }
  ...
  void onRegistrationFailure(
    ...
    showSnakBarMessage(
        'PhoneNumber $phoneNumber registeration fail ${error.toString()}');
  }
